...
async function eccEncrypt(publicKey, message)  {
    let tmp = await ECC_Encrypt(publicKey,message);
    console.log(tmp);
    let blob =[
...
async function eccDecrypt(privateKey, blob){
    ...
    let message = await eccrypto.decrypt(privateKey, cypher);
